...
ui-build-pkg:
	...
	$(SUDO) docker run --name scope-pkg-build -it \
	-v $(shell pwd)/client/app/:/home/weave/app/ \
	-v $(shell pwd)/tmp:/home/weave/tmp \
	$(SCOPE_UI_BUILD_IMAGE) npm run bundle && \
	aws s3 cp $(shell pwd)/tmp/weave-scope.tgz s3://weaveworks-npm-modules/weave-scope/ --acl public-read
